...
class MaskedMultiHeadAttention(nn.Module):
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        attention_output, attention_weights = self.attention(Q_proj, K_proj, V_proj, mask) #here attention_weights is passed here because it is needed for visualization and analysis of the attention mechanism, allowing us to see which parts of the input sequence the model is focusing on when making predictions.
